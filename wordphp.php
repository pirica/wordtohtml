...
class WordPHP
{
	...
	private function getListFormating(&$xml,$Tstyle)
	{
		...
		$ListF = " style='".$font.$Fsize.$bmar.$amar."'"; // formatting for list elements
	$ListOF = " style='".$bmar.$amar."'"; //formatting for list header
		...
		switch($zltype[$zzcrossref][$zzlistilvl]) {
			case 'decimal':
				$PSret['openF'] = "<ol ".$zstnum.$ListOF."><li".$ListF.">";
				$PSret['openL'] = "<li".$ListF.">";
				$PSret['closeF'] = "</span></li>";
				$PSret['closeL'] = "</span></li></ol>";
				break;
			case 'lowerLetter':
				$PSret['openF'] = "<ol type = 'a' ".$zstnum.$ListOF."><li".$ListF.">";
				$PSret['openL'] = "<li".$ListF.">";
				$PSret['closeF'] = "</span></li>";
				$PSret['closeL'] = "</span></li></ol>";
				break;
			case 'upperLetter':
				...
				break;
			case 'lowerRoman':
				$PSret['openF'] = "<ol type = 'i' ".$zstnum.$ListOF."><li".$ListF.">";
				$PSret['openL'] = "<li".$ListF.">";
				$PSret['closeF'] = "</span></li>";
				$PSret['closeL'] = "</span></li></ol>";
				break;
			case 'upperRoman':
				...
				break;
			case 'bullet':
				$PSret['openF'] = "<ul".$ListOF."><li".$ListF.">";
				$PSret['openL'] = "<li".$ListF.">";
				$PSret['closeF'] = "</span></li>";
				$PSret['closeL'] = "</span></li></ul>";
				break;
		}

		$PSret['level'] = $zzlistilvl;
		$PSret['listnum'] = $zzlistnumId;
		return $PSret;
			...
			if ($paragraph->nodeType == XMLREADER::ELEMENT && $paragraph->name === 'w:r') {
				...
				if ($list_format['listnum'] <> ''){ // Check to see if the element is part of the list. If so increments a counter (To enable to determine when the end of the list is reached)
					if ($listcount == ''){
						...
						$listcount[$list_format['listnum']] = $listcount[$list_format['listnum']] + 1;
					}
				}
				...
				if ($Zlist == 0){
					$text .= ($zta == '' && $zct == 0) ? "<p".$Dstyle['Pform'].">" : $zta.">";
				} else {
					$zlistPF = ($zta == '' && $zct == 0) ? "<p".$Dstyle['Pform'].">" : $zta.">";
				}
				$paragraph->next();
			}
