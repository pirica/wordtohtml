...
class WordPHP
{
				...
				while ($st2->read()) {
					if($st2->name == "w:spacing") { // Checks for paragraph spacing
						if ($st2->getAttribute("w:before") <>''){
							$Rstyle['DMtop'] =  "margin-top: ".round($st2->getAttribute("w:before")/13)."px;";
						}
						if ($st2->getAttribute("w:after") <>''){
							$Rstyle['DMbot'] =  "margin-bottom: ".round($st2->getAttribute("w:after")/13)."px;";
						}
					}
					if($st2->name == "w:sz") {
						$Rstyle['DFontS'] = round($st2->getAttribute("w:val")/$FSFactor,2);
					}
					if($st2->name == "w:color") {
			...
			if ($reader1->nodeType == XMLREADER::ELEMENT && $reader1->name == 'w:style') { //Get style settings
				if ($reader1->getAttribute("w:styleId") == $style){
					...
					while ($st1->read()) {
						...
						if($st1->name == "w:sz") {
							$Rstyle['FontS'] = round($st1->getAttribute("w:val")/$FSFactor,2);
						}
						if($st1->name == "w:caps") {
						...
						if($st1->name == "w:spacing") { // Checks for paragraph spacing
							if ($st1->getAttribute("w:before") <>''){
								$Rstyle['Mtop'] =  "margin-top: ".round($st1->getAttribute("w:before")/13)."px;";
							}
							if ($st1->getAttribute("w:after") <>''){
								$Rstyle['Mbot'] =  "margin-bottom: ".round($st1->getAttribute("w:after")/13)."px;";
							}
						}
						if($st1->name == "w:ind") { // Checks for paragragh indent
							if ($st1->getAttribute("w:left") <>''){
								$Rstyle['Ileft'] =  "padding-left: ".round($st1->getAttribute("w:left")/13)."px;";
							}
							if ($st1->getAttribute("w:right") <>''){
								$Rstyle['Iright'] =  "padding-right: ".round($st1->getAttribute("w:right")/13)."px;";
							}
							if ($st1->getAttribute("w:hanging") <>''){
								$Rstyle['Ihang'] =  "text-indent: -".round($st1->getAttribute("w:hanging")/13)."px;";
							}
							if ($st1->getAttribute("w:firstLine") <>''){
								$Rstyle['Ifirst'] =  "text-indent: ".round($st1->getAttribute("w:firstLine")/13)."px;";
							}
						}
	...
	{	
		...
		while ($reader->read()) {
			...
			if($reader->name == "w:sz") {
				$Lstyle['FontS'] = round($reader->getAttribute("w:val")/$FSFactor,2);
			}
	...
	private function getListFormating(&$xml,$Tstyle)
	{
		...
		while ($reader->read()){
			...
			if($reader->name == "w:sz") { // get font size for list numbering
				$Fsize = " font-size: ".round($reader->getAttribute("w:val")/$FSFactor,2)."rem;";
			}
