...
class WordPHP
{
	...
	private function findstyles($style)
	{
		...
		// look for required style
			...
			if ($reader1->nodeType == XMLREADER::ELEMENT && $reader1->name == 'w:docDefaults') { //Get document default settings
				...
				while ($st2->read()) {
					...
					if($st2->name == "w:rFonts" and $st2->getAttribute("w:ascii")) {
						$DF = $st2->getAttribute("w:ascii");
						if (substr($DF,0,9) == 'Helvetica'){
							$DF = 'Helvetica';
						}
						$Rstyle['DFont'] = " font-family: ".$DF.";";
					}
				}
			}
				...
				if ($reader1->getAttribute("w:styleId") == $style){
					...
					while ($st1->read()) {
						...
						if($st1->name == "w:numId") { // Get paragraph numbering ref
							$Rstyle['parnum'] = $st1->getAttribute("w:val");
						}
		...
		if ($FontTheme AND $Rfont['major']){
			$Rstyle['ThFont'] = " font-family: ".$Rfont['major'].";";
		} else if (! $FontTheme AND $Rfont['minor']){
			$Rstyle['ThFont'] = " font-family: ".$Rfont['minor'].";";
		}
		return $Rstyle;
	}
	...
	**/ 
	...
	{	
		...
		$Nstyle = $this->findstyles('Normal'); // get the default 'normal' styles for this document
		...
		if ($Lstyle['Font']){
			...
			$f .= $Nstyle['Font'];
		} else if ($Rstyle['ThFont']){
			$f .= $Rstyle['ThFont'];
		} else if ($Rstyle['DFont']){
			$f .= $Rstyle['DFont'];
		}
		if ($Lstyle['FontS']){
			$Fsize = $Lstyle['FontS'];
	 ...
	 */
	private function getParagraph(&$paragraph,$Tstyle,$imgcss)
	{
		$text = '';
		$list_format=array();
		$zst = array();
		...
		while ($paragraph->read()) {
			...
			if ($paragraph->nodeType == XMLREADER::ELEMENT && $paragraph->name === 'w:r') {
				$Pcheck = $this->checkFormating($paragraph,$list_format['style'],$Tstyle,$imgcss); //check if this element is a page break and if so ignore this element
				if ($Pcheck['Pbreak'] == ''){
					if ($Pformat == 'Y'){
...
// ------------------- START OF TABLE PROCESSING ------------------------
	 ...
	 */
	private function checkTableFormating(&$xml,$imgcss)
	{
			...
			if ($xml->nodeType == XMLREADER::ELEMENT && $xml->name === 'w:tr') { //find and process a table row
				...
				while ($tr->read()) {
					...
					if ($tr->nodeType == XMLREADER::ELEMENT && $tr->name === 'w:tc') { //get cell borders and cell text and its formatting
						$tc = $this->processTableRow(trim($tr->readOuterXML()),$Tstyle,$imgcss);
					}
					$style = '';
	 ...
	 */
	private function processTableRow($content,$Tstyle,$imgcss)
	{
		$tc = new XMLReader;
		$tc->xml($content);
		$ct = array();
		...
		while ($tc->read()) {
			...
			if ($tc->nodeType == XMLREADER::ELEMENT && $tc->name === "w:p") {  // get cell text and its formatting
				...
				$p = $tc->readOuterXML();
				$paragraph->xml($p);
				$ct['cell'] .= $this->getParagraph($paragraph,$Tstyle,$imgcss);
			}
	...
	public function readDocument($filename,$imgcss)
	{
		...
		// look for new paragraphs or table
			...
			if ($reader->nodeType == XMLREADER::ELEMENT && $reader->name === 'w:tbl') { // finds and gets tables
				$paragraph->xml($p);
				$text .= $this->checkTableFormating($paragraph,$imgcss);
				$reader->next();
			}
				...
				$text .= $this->getParagraph($paragraph,'',$imgcss);
				$reader->next();
			}
		}
